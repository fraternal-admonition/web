...
### 1.1 Project Setup
- Repo, linting, formatting, CI, environment templates.
- **Accept:** `pnpm build` & `pnpm dev` OK; `.env.example` documented.

### 1.2 Auth & Public Profiles
- Sign up / Sign in (email+password), email verification.
- On signup: create `public.users` row (role `USER`, optional `display_id`).
- **Tables:** `public.users` (FK to `auth.users`).
- **Accept:** Verified users can access dashboard; ban flag respected in app middleware.

### 1.3 CMS (Pages & Settings)
- Admin CRUD for `cms_pages`, upload `cms_assets`, edit `cms_settings` (e.g., site lock, banners).
- Public render of Home, About, Rules, FAQ, Contact.
